...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	// Watch for changes to secondary resource Pods and requeue the owner ExternalDatabase
...
func (r *ReconcileExternalDatabase) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	db := &apiv1alpha1.ExternalDatabase{}
	...
	if spec.AdminPassword.Namespace == "" {
		spec.AdminPassword.Namespace = request.Namespace
	}

	secret := &corev1.Secret{}
	err = r.client.Get(context.TODO(),
		types.NamespacedName{Name: spec.AdminPassword.Name, Namespace: spec.AdminPassword.Namespace},
		secret)
	if err != nil {
		if errors.IsNotFound(err) {
	...
	pwBytes, ok := secret.Data["mysql-password"]
	if !ok {
		reqLogger.Info("Secret does not container mysql-password")
		return reconcile.Result{}, err
	}

	pw := string(pwBytes)
	edbi, err := edb.NewMySQL(db.Spec.AdminUser, pw, db.Spec.Host)
	if err != nil {
	...
	// We have been able to contact the DB
	db.Status.Reachable = true
